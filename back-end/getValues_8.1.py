...
def getTimeGap(obj, period, times, format):
    ...
    for i in range(len(times)):
        # print(isinstance(times[i], str), times[i])
...
def dfInterpolate(df):
    ...
    # df = df.reindex_like(df_reindex)

    df = df.infer_objects()
    df = df.interpolate(method='linear', limit_direction='forward', axis=0)
    df.datetime = df.index
    # df.to_csv('filled_stats.csv', index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datetime', type=str, default="2022-06-02 00:00:00",
